...
/**
 * Send-pump tuning (speed). TARGET_SEND_CHUNK intentionally diverges from the
 * conservative 16 KiB message size: the wire is chunk-size-agnostic (the
 * receiver just concatenates ArrayBuffers into a Blob), so we send far fewer,
 * much larger SCTP messages to push a Chrome<->Chrome channel toward the WiFi
 * link rate instead of stalling on ~16x more micro-reads/messages.
 *
