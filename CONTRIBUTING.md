...
6. **`SEND_HIGH_WATER` (in `peer.ts`) must stay well below 16 MiB.** Three different size limits get conflated here, so name them before you touch the constant (`docs/research-2026-07.md` §1): the **64 KiB** default per-message `maxMessageSize` (RFC 8841 §6.1), the **256 KiB** SCTP *association* send buffer (usrsctp / Chromium `kSendBufferSize`), and the **16 MiB** `bufferedAmount` *send-queue* cap (libwebrtc `MaxSendQueueSize`; Blink throws `OperationError` above it). Backpressure keys off the last one, and `bufferedAmount` can never *reach* it because `send()` throws first — so a ≥16 MiB high-water mark disables backpressure entirely and large transfers die mid-send. It's 8 MiB today, and the check counts the chunk about to be sent. Don't "tune" this upward.
