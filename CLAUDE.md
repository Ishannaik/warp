...
- **$0 and no credit card.** No paid infrastructure. The canonical production deployment stays on Cloudflare free (Pages + Workers + Durable Objects), with no card on the account so nothing can ever bill. Optional frontend self-host targets documented in `docs/SELF-HOSTING.md` are allowed only when their current $0/no-card status has been verified; they must not replace the canonical Cloudflare deploy or move signaling/file bytes onto a paid relay. **No TURN relay by default** (STUN-only) — restrictive/cross-network NAT gets an honest error, never a silent paid relay. If a relay is ever added it must be unmetered/no-bill (self-hosted coturn, or CF TURN with no card).
...
- **`SEND_HIGH_WATER` (peer.ts) must stay well BELOW 16 MiB.** Don't conflate the three SCTP size limits (`docs/research-2026-07.md` §1): the 64 KiB *default* `maxMessageSize` (RFC 8841 §6.1), the 256 KiB SCTP *association send buffer* (usrsctp/Chromium default), and the 16 MiB *`bufferedAmount` send-queue cap* (libwebrtc `MaxSendQueueSize`; Blink's `send()` throws `OperationError` above it). Backpressure keys off the last one: `bufferedAmount` can never *reach* 16 MiB because `send()` throws first, so a ≥16 MiB high-water mark disables backpressure entirely and every large transfer dies mid-send (~40% of a 50 MB file). It's 8 MiB now; the check also counts the chunk about to be sent. (256 KiB messages intentionally exceed RFC 8831 §6.6's 16 KB SHOULD — fine for a single bulk stream, still capped to the peer's negotiated limit.)
...
- **Piece manifest (#137) verifies each piece, not just the byte count.** For a *fresh* file in the size window (`shouldManifest` in `pieceManifest.ts`: ≥1 MiB, ≤2 GiB), the sender rides a per-piece SHA-256 manifest in `file-begin`; the receiver feeds plaintext through a `PieceVerifier` and appends only VERIFIED pieces to the sink, re-requesting a bad index via `piece-request`/`piece` (one piece, never a full resend). Tiny/huge files and resumed receives keep the legacy trust-the-stream path. The manifest frame must stay under the negotiated `maxMessageSize`, so `choosePieceSize` grows the piece size to cap the hash count at `MAX_PIECES` (512). The channel is reliable+DTLS, so this targets storage/implementation corruption, not an adversarial wire.
